  ...
  build: function* (
    ...
    switch (backend) {
      ...
      case 'root':
        render = path.join(
          releasePath,
          'node_modules/goblin-blacksmith/lib/child-renderer/renderers',
          `${backend}.js`
        );
        break;
      ...
      default:
        throw new Error(
          `missing backend parameter, must be 'component', 'root' or 'pdf'`
        );
    }

    if (!fs.existsSync(render)) {
      throw new Error(`render ${render} is not available`);
    }
  ...
  *_renderIndex(quest, backend, mainGoblin, componentPath) {
    ...
    if (xBlacksmithConfig && xBlacksmithConfig.renderers[backend]) {
      ...
      if (xBlacksmithConfig.renderers[backend][filename]) {
        ...
        const renderIndex = path.join(
          resourcesPath,
          xBlacksmithConfig.outputDir || 'blacksmith',
          `.${filename}`
        );
        if (fs.existsSync(renderIndex)) {
          return [renderIndex, filename];
        }
        publicPath = xBlacksmithConfig.renderers[backend][filename].publicPath;
      }
    }

    if (!quest.hasAPI('webpack.pack')) {
      throw new Error(
        `the ${backend} renderer for ${componentPath} and webpack are not available`
      );
    }
  ...
  renderComponent: function* (
    ...
    yield backendRendererLock.lock(`${quest.goblin.id}.renderComponent`);
    quest.defer(() =>
      backendRendererLock.unlock(`${quest.goblin.id}.renderComponent`)
    );

    return yield quest.me._render({
      backend: isRoot ? 'root' : 'component',
      mainGoblin,
